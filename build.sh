...
rm -rf build/ && mkdir -p build/ && cd build && cmake -GNinja -DCMAKE_INSTALL_PREFIX=deploy -DCMAKE_TOOLCHAIN_FILE=toolchains/clang-toolchain.cmake .. && ninja -v -j $(nproc) install
