...
public:
    explicit Impl(std::unique_ptr<TestInstance> internalTest)
        : test(std::move(internalTest)) {}

    std::unique_ptr<TestInstance> test;
};
...
detail::TestHandle detail::RuntimeBridge::makeTest(
    ...
    return TestHandle{std::make_unique<TestHandle::Impl>(
        std::make_unique<TestInstance>(std::string{name}, location, std::move(options),
                                       TestFunction{std::move(function)})
    )};
}
...
detail::TestHandle detail::RuntimeBridge::makeTest(
    ...
    return TestHandle{std::make_unique<TestHandle::Impl>(
        std::make_unique<TestInstance>(std::string{name}, location, std::move(options),
                                       TestFunction{std::move(function)}, fixtureType)
    )};
}
