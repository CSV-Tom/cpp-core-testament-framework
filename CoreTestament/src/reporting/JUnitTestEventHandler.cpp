...
JUnitTestEventHandler::JUnitTestEventHandler(std::filesystem::path reportPath)
    : mOutputPath(std::move(reportPath)), mOutputPathIsFixed(true) {}

std::expected<void, std::string> JUnitTestEventHandler::configure(Arguments arguments) {
    if (mOutputPathIsFixed) return {};

    mOutputPath.reset();
    mSuiteResults.clear();
    mEnvironmentErrors.clear();
    mReportWritten = false;
    mWriteError.clear();
...
void JUnitTestEventHandler::onStartReport(unsigned int suiteCount) {
    if (!mOutputPath) return;
    mSuiteResults.clear();
    mSuiteResults.reserve(suiteCount);
    mReportWritten = false;
    mWriteError.clear();
}
...
void JUnitTestEventHandler::onEnvironmentError(std::string_view phase, std::string_view message) {
    if (mOutputPath) mEnvironmentErrors.emplace_back(std::string{phase} + ": " + std::string{message});
}
...
JUnitTestEventHandler::SuiteResult& JUnitTestEventHandler::suiteResult(const SuiteInfo& suite) {
    const auto existing = std::ranges::find(mSuiteResults, suite.name, &SuiteResult::name);
    if (existing != mSuiteResults.end()) return *existing;
    mSuiteResults.push_back({suite.name, suite.location, {}, {}});
    return mSuiteResults.back();
}

void JUnitTestEventHandler::writeReport() {
    if (!mOutputPath) return;
    std::size_t testCount = mEnvironmentErrors.size();
    std::size_t failureCount = 0;
    std::size_t skippedCount = 0;
    std::size_t errorCount = mEnvironmentErrors.size();
    double totalDuration = 0.0;

    for (const auto& suite : mSuiteResults) {
    ...
    if (const auto parent = mOutputPath->parent_path(); !parent.empty()) {
        std::filesystem::create_directories(parent, directoryError);
    }
    if (directoryError) {
        mWriteError = "Cannot create JUnit output directory: " + directoryError.message();
        return;
    }

    std::ofstream output(*mOutputPath);
    if (!output) {
        mWriteError = "Cannot open JUnit output file: " + mOutputPath->string();
        return;
    }
    ...
    if (!mEnvironmentErrors.empty()) {
        output << "  <testsuite name=\"global environment\" tests=\""
               << mEnvironmentErrors.size() << "\" failures=\"0\" errors=\""
               << mEnvironmentErrors.size() << "\" skipped=\"0\" time=\"0.000000000\">\n";
        for (const auto& error : mEnvironmentErrors) {
    ...
    output.close();
    if (!output) {
        mWriteError = "Failed while writing JUnit output file: " + mOutputPath->string();
        return;
    }
    mReportWritten = true;
}
