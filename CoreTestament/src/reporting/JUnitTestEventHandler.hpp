...
public:
    ...
    explicit JUnitTestEventHandler(std::filesystem::path reportPath);
...
private:
    ...
    std::optional<std::filesystem::path> mOutputPath;
    bool mOutputPathIsFixed{false};
    std::vector<SuiteResult> mSuiteResults;
    std::vector<std::string> mEnvironmentErrors;
    bool mReportWritten{false};
    std::string mWriteError;
};
